...
public class UserSwitchTransitionViewController extends OverlayViewController {
    ...
    private boolean mShouldSkipTimeout = false;

    private final Runnable mWindowShownTimeoutCallback = () -> {
        if (mShouldSkipTimeout) {
            return;
        }
    ...
    @MainThread
    void showSwitchingUI(@UserIdInt int newUserId) {
        if (mPreviousUserId == newUserId || mTransitionViewShowing) {
            return;
        }
        mNewUserId = newUserId;
        mTransitionViewShowing = true;
        if (DEBUG) {
            Log.d(TAG, "Showing switching UI");
        }
        start();
    ...
    void setShouldSkipTimeout(boolean skipTimeout) {
        mShouldSkipTimeout = skipTimeout;
    }
